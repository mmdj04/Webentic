...
description: Displays a control for easier uploading of files directly to Webentic Storage
---
...
This block assumes that you have already installed a Webentic client for Nuxt from the previous step.
...
import { useWebenticUpload } from '@/composables/use-webentic-upload'

const upload = useWebenticUpload({
  bucketName: 'test',
  path: 'test',
...
| ------------------ | ---------- | ---------- | ---------------------------------------------------- |
| `bucketName`       | `string`   | `null`     | The name of the Webentic Storage bucket to upload to |
