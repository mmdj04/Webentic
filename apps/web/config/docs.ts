...
export const docsConfig = {
  sidebarNav: [
    {
      ...gettingStarted,
      items: gettingStarted.items.map((item) => ({ title: item.title, href: item.href, items: [] })),
    },
    {
      ...aiEditorsRules,
      items: aiEditorsRules.items.map((item) => ({ title: item.title, href: item.href, items: [] })),
    },
    {
      ...platformBlocks,
      items: platformBlocks.items.map((item) => ({ title: item.title, href: item.href, items: [] })),
    },
  ],
}
