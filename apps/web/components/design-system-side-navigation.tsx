'use client'

import Link from 'next/link'
import { usePathname } from 'next/navigation'

import { cn } from 'ui'
import { ThemeSwitcherDropdown } from './theme-switcher-dropdown'
import { docsConfig } from '@/config/design-system-docs'

function DesignSystemSideNavigation() {
  const pathname = usePathname()

  return (
    <nav className="flex flex-col h-full min-w-[220px]">
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <Link href="/">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="109"
              height="113"
              viewBox="0 0 109 113"
              fill="none"
              className="w-6 h-6"
            >
              <path
                d="M63.7076 110.284C60.8481 113.885 55.0502 111.912 54.9813 107.314L53.9738 40.0625L99.1935 40.0625C107.384 40.0625 111.952 49.5226 106.859 55.9372L63.7076 110.284Z"
                fill="url(#paint0_linear)"
              />
              <path
                d="M63.7076 110.284C60.8481 113.885 55.0502 111.912 54.9813 107.314L53.9738 40.0625L99.1935 40.0625C107.384 40.0625 111.952 49.5226 106.859 55.9372L63.7076 110.284Z"
                fill="url(#paint1_linear)"
                fillOpacity="0.2"
              />
              <path
                d="M45.317 2.07103C48.1765 -1.53037 53.9745 0.442937 54.0434 5.041L54.4849 72.2922H9.83113C1.64038 72.2922 -2.92775 62.8321 2.1655 56.4175L45.317 2.07103Z"
                fill="#3ECF8E"
              />
              <defs>
                <linearGradient id="paint0_linear" x1="53.9738" y1="54.9738" x2="94.1635" y2="71.8293" gradientUnits="userSpaceOnUse">
                  <stop stopColor="#249361" />
                  <stop offset="1" stopColor="#3ECF8E" />
                </linearGradient>
                <linearGradient id="paint1_linear" x1="36.1558" y1="30.5779" x2="54.4844" y2="65.0804" gradientUnits="userSpaceOnUse">
                  <stop />
                  <stop offset="1" stopOpacity="0" />
                </linearGradient>
              </defs>
            </svg>
          </Link>
          <ThemeSwitcherDropdown />
        </div>
        <Link href="/design-system" className="mb-4 block">
          <h1>Webentic Design System</h1>
        </Link>
      </div>
      {docsConfig.sidebarNav.map((group) => (
        <div key={group.title} className="pb-6">
          <div className="font-mono uppercase text-xs text-foreground-lighter/75 mb-2 px-6 tracking-widest">
            {group.title}
          </div>
          <div className="space-y-0.5">
            {group.items.map((item) => {
              const href = item.href.startsWith('/')
                ? `/design-system${item.href}`
                : `/design-system/${item.href}`
              const isActive = pathname === href

              return (
                <Link
                  key={item.title}
                  href={href}
                  className={cn(
                    'relative flex items-center gap-2 rounded-md px-6 py-1 text-sm transition-colors',
                    isActive
                      ? 'text-foreground bg-surface-200 font-medium'
                      : 'text-foreground-light hover:text-foreground hover:bg-surface-100'
                  )}
                >
                  {item.title}
                  {item.label && (
                    <span className="text-[10px] font-medium text-brand-600 bg-brand-200 px-1 rounded">
                      {item.label}
                    </span>
                  )}
                </Link>
              )
            })}
          </div>
        </div>
      ))}
    </nav>
  )
}

export default DesignSystemSideNavigation
